...
-- 2.3.4 ORE: compare 2 ciphertext kiểu BIGINT
CREATE FUNCTION udf_ore_cmp(bigint, bigint)
RETURNS integer
AS 'MODULE_PATHNAME', 'ore_cmp_int8'
LANGUAGE C STRICT IMMUTABLE;

-- Các wrapper tiện lợi dùng trong SQL ore_cmp_int8

CREATE FUNCTION udf_ore_lt(bigint, bigint)
RETURNS boolean
LANGUAGE SQL IMMUTABLE STRICT
AS $$ SELECT udf_ore_cmp($1, $2) < 0 $$;

CREATE FUNCTION udf_ore_le(bigint, bigint)
RETURNS boolean
LANGUAGE SQL IMMUTABLE STRICT
AS $$ SELECT udf_ore_cmp($1, $2) <= 0 $$;

CREATE FUNCTION udf_ore_gt(bigint, bigint)
RETURNS boolean
LANGUAGE SQL IMMUTABLE STRICT
AS $$ SELECT udf_ore_cmp($1, $2) > 0 $$;

CREATE FUNCTION udf_ore_ge(bigint, bigint)
RETURNS boolean
LANGUAGE SQL IMMUTABLE STRICT
AS $$ SELECT udf_ore_cmp($1, $2) >= 0 $$;

CREATE FUNCTION udf_ore_eq(bigint, bigint)
RETURNS boolean
LANGUAGE SQL IMMUTABLE STRICT
AS $$ SELECT udf_ore_cmp($1, $2) = 0 $$;
